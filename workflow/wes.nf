...
include { PICARD_COLLECTHSMETRICS } from '../nf-bioskryb-utils/modules/picard/collecthsmetrics/main.nf' addParams( timestamp: params.timestamp )
...
workflow WES_WF {
    ...
    main:
        ...
        */
        ...
        ch_picard_version = PICARD_COLLECTHSMETRICS.out.version
        ...
        ch_metrics = SENTIEON_DRIVER_METRICS_WF.out.metrics_tuple
                .join(PICARD_COLLECTHSMETRICS.out.metrics_tuple, by: 0)
                .map{ sample, metrics1, metrics2 -> [sample, metrics1 + [metrics2]] }

        ch_custom_data_processing_input = SENTIEON_SECONDARY_WF.out.dedup_metrics
                                                .combine (
                                                            ch_bam
                                                                .join( ch_metrics, by: 0),
                                                            by: 0
                                                         )
...
workflow.onComplete {
    ...
    if (file("$params.tmp_dir/vcf_annotated_files.txt").exists()) {
        output["output"]["vcf-annotated"]   = [:]
        vcf_annotated_outfile = file("$params.tmp_dir/vcf_annotated_files.txt")
        vcf_annotated_outfile_lines = vcf_annotated_outfile.readLines()
        for ( vcf_annotated_line : vcf_annotated_outfile_lines ) {
            def (sample_name, vcf_annotated_path) = vcf_annotated_line.split('\t')
            output["output"]["vcf-annotated"][sample_name] = [:]
            output["output"]["vcf-annotated"][sample_name]["vcf-annotated"] = vcf_annotated_path
        }
    }
